...
getVideoSize: func [] [round/to ((size? to-file fileName) / 1000000.0) 0.01]

;*************************** Main ******************************
view win: layout [
	title "Red Camera with ffmpeg"
	...
	return
	camList: drop-list 210 
		on-create [face/data: cam/data]
		on-select [vDevice: aDevice: camList/selected - 1 fDevice/text: form vdevice]
	fDevice: field 30 
	text 40 bold "Size"
	cSize: field 70
	text 30 bold "FPS"
	...
	sF: field 130 right on-time [face/text: form getVideoSize
		append face/text " Mo" recycle ]
	do [getPlatform camList/selected: 1 vDevice: 0 fDevice/text: form vDevice 
	tF/rate: none sl/data: 1.0 ffps/text: form frameRate append ffps/text " fps"]
]